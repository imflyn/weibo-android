 ...
 * 
 * <pre>
 * OAuthV2Client.setQHttpClient(QHttpClient qHttpClient)
 * </pre>
 * 
 * <br>
 * 为本工具类指定http管理器
...
public class OAuthV2Client
{
    ...
    {
        if ((!QStrOperate.hasValue(authorizeCode)) || (!QStrOperate.hasValue(openid)) || (!QStrOperate.hasValue(openkey)))
        {
            return false;
        }
     ...
     * 
     * @param responseData
     *            格式：code=CODE&openid=OPENID&openkey=OPENKEY
     * @param oAuth
     * @return
     */
    ...
    public static boolean accessToken(OAuthV2 oAuth) throws Exception
    {
        ...
        if (!parseAccessToken(responseData, oAuth))
        {// Access Token 授权不通过
            oAuth.setStatus(3);
            return false;
        } else
     ...
     * 
     * @param responseData
     *            格式：access_token=ACCESS_TOKEN&expires_in=60&name=NAME
     * @param oAuth
     * @return
     */
     ...
     * 
     * @param responseData
     *            格式：access_token=ACCESS_TOKEN&expires_in=60&openid= OPENID
     *            &openkey= OPENKEY
     * @param oAuth
     * @return
     */
    public static boolean parseAccessTokenAndOpenId(String responseData, OAuthV2 oAuth)
    {
        ...
        String strRefresh_token = tokenArray[4];
        ...
        String[] refresh_token = strRefresh_token.split("=");
        if (refresh_token.length < 2)
        {
            return false;
        }
        oAuth.setRefreshToken(refresh_token[1]);
