...
public class ImageZoomDialog extends BaseActivity implements OnTouchListener, OnClickListener
{
        ...
        btnSave.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                try
                {
                    ...
                    ImageUtils.saveImageToSD(ImageZoomDialog.this, url + ".jpg", bitmap, 100);
    ...
    private void initData()
    {
        ...
        memoryCache = MemoryCache.getMemoryCache(this);
        ...
        new Thread()
        {
            ...
            public void run()
            {
                ...
                {
                    bmp = BitmapFactory.decodeStream(imgView.getResources().openRawResource(R.drawable.widget_dface));
                }
                if (bmp == null)
                {
                    // 是否有缓存图片
                    // Environment.getExternalStorageDirectory();返回/sdcard
                    bmp = memoryCache.getBitmapFromCache(imgURL);
                    if (null != bmp)
                    {
                        bmp = fileCache.getImage(imgURL);
                    }
                    ...
                    // if (file.exists()) {
                    // bmp =
                    // ImageUtils.getBitmap(imgView.getContext(),filename);
                    // }
                }
                if (bmp == null)
                {
                    ...
                    if (bmp != null)
                    {
                        try
                        {
                            ...
                            // bmp);
                            memoryCache.addBitmapToCache(imgURL, bmp);
                            fileCache.saveBitmap(bmp, imgURL);
                        } catch (Exception e)
                        {
                            e.printStackTrace();
                        }
